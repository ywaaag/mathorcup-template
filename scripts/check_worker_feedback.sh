...
usage() {
    echo "Usage: bash scripts/check_worker_feedback.sh [--task <task_id> | --file <path>] [--target <dir>]" >&2
}
...
args=(check-feedback --root "$TARGET_DIR")
...
if [[ -n "$TASK_ID" ]]; then
    ...
    emit_workflow_event "$SCRIPT_DIR" "$TARGET_DIR" \
        --event-type review.checked \
        --task "$TASK_ID" \
        --actor main_brain \
        --owner "$OWNER" \
        --artifact "$feedback_path" >/dev/null
fi
